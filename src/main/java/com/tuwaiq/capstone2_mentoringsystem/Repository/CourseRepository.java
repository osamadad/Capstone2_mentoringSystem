...
public interface CourseRepository extends JpaRepository<Course,Integer> {
    ...
    @Query("select courses from Course courses where courses.instructorId in (select instructor from Instructor instructor where instructor.city=?1) and courses.adminStatus='approved'")
    ...
    @Query("select course from Course course where course.instructorId in (select instructor from Instructor instructor where instructor.gender=?1) and course.adminStatus='approved'")
    List<Course> getCoursesByInstructorGender (String gender);

    @Query("select course from Course course where course.instructorId in (select intructor from Instructor intructor where intructor.yearsOfExperience>=?1 and intructor.categoryId=?2) and course.adminStatus='approved'")
    List<Course> getCoursesByInstructorYearOfExperiencesAndCategory(Integer yearOfExperiences, Integer categoryId);

    @Query("select course from Course course where course.instructorId in (select intructor from Instructor intructor where intructor.rating>=?1 and intructor.categoryId=?2) and course.adminStatus='approved'")
    List<Course> getCoursesByInstructorRatingAndCategory(Integer rating, Integer categoryId);

    @Query("select course from Course course where course.instructorId in (select intructor from Instructor intructor where intructor.registerDate>=?1) and course.adminStatus='approved'")
    ...
    @Query("select course from Course course where course.id in (select courseSession from CourseSession courseSession where courseSession.startDate>=?1) and course.adminStatus='approved'")
    List<Course> getCoursesAfterStartDate(LocalDate startDate);

    @Query("select course from Course course where course.id in (select courseSession from CourseSession courseSession where courseSession.endDate<=?1) and course.adminStatus='approved'")
    List<Course> getCoursesBeforeEndDate(LocalDate endDate);

    @Query("select course from Course course where course.id in (select courseSession from CourseSession courseSession where courseSession.startDate>=?1 and CourseSession.endDate<=?2) and course.adminStatus='approved'")
